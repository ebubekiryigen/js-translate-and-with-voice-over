const fromLang = document.querySelector('#form-lang')
const toLang   = document.querySelector('#to-lang')
const btnTra   = document.querySelector('#btnTranslate')
...
btnTra.addEventListener("click", ()=>{
    ...
    let to   = toLang.value
    const langApi = `https://api.mymemory.translated.net/get?q=${text}!&langpair=${from}|${to}`
    fetch(langApi)
    .then(res  => res.json())
    .then(data => {
